...
class EP_WooCommerce_Module extends EP_Module {
	public $slug = 'woocommerce';

	public $requires_install_reindex = true;

	public function setup() {
		add_filter( 'ep_sync_insert_permissions_bypass', array( $this, 'bypass_order_permissions_check' ), 10, 2 );
		add_filter( 'ep_elasticpress_enabled', array( $this, 'blacklist_coupons' ), 10 ,2 );
		add_filter( 'ep_formatted_args',  array( $this, 'formatted_args' ), 10, 2 );
		add_filter( 'ep_indexable_post_types', array( $this, 'post_types' ), 10, 1 );
		add_filter( 'ep_prepare_meta_allowed_protected_keys', array( $this, 'whitelist_meta_keys' ), 10, 2 );
		add_filter( 'woocommerce_shop_order_search_fields', array( $this, 'shop_order_search_fields' ) );
		add_filter( 'woocommerce_layered_nav_query_post_ids', array( $this, 'convert_post_object_to_id' ), 10, 4 );
		add_filter( 'woocommerce_unfiltered_product_ids', array( $this, 'convert_post_object_to_id' ), 10, 4 );
		add_filter( 'ep_sync_taxonomies', array( $this, 'whitelist_taxonomies' ), 10, 2 );
		add_filter( 'ep_post_sync_args_post_prepare_meta', array( $this, 'remove_legacy_meta' ), 10, 2 );
		add_action( 'pre_get_posts', array( $this, 'translate_args' ), 11, 1 );
		add_filter( 'ep_admin_wp_query_integration', array( $this, '__return_true' ) );
		add_filter( 'ep_indexable_post_status', array( $this, 'get_statuses' ) );
	}
	 ...
	 * @return  array
	 */
	public function remove_legacy_meta( $post_args, $post_id ) {
...
return new EP_WooCommerce_Module;
